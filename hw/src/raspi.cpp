...
namespace telescope {
    ...
    bool RasPi::send_time_mode(const TimeModePayload& p) {
        return send_packet(PACKET_TIME_MODE, reinterpret_cast<const uint8_t*>(&p), sizeof(p));
    }

    bool RasPi::send_search_guidance(const SearchGuidancePayload& p) {
